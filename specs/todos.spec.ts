...
describe("TODOS", () => {
  ...
  it("TODOS request", async () => {
    const data = {
      title: "My first todos request",
      body: "This is my first todos request",
      userId: 1001,
    };
    const res = await request.post("/todos").send(data);
    expect(res.statusCode).toEqual(201),
      expect(res.body.title).toEqual("My first todos request");
  });
  ...
  it("PATCH request with title version 1", async () => {
    ...
    const getRes = await request.get("/todos/1");
  ...
  it("PATCH request with title version 3", (done) => {
    ...
    request.get("/todos/1").end((err, res) => {
